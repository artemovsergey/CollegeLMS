...
public class ExamService(AppDbContext db) : IExamService
{
    ...
    )
    {
        ...
        if (!exam)
            return Result<RetakeResponse>.Fail("Экзамен не найден", 404);

        if (!await db.Students.AnyAsync(s => s.Id == request.StudentId, ct))
            return Result<RetakeResponse>.Fail("Студент не найден", 404);
    ...
    )
    {
        var retakes = await db
            ...
            .ToListAsync(ct);
        return Result<List<RetakeResponse>>.Ok(retakes.Select(r => r.ToDto()).ToList());
    }
    ...
    )
    {
        ...
        if (!Enum.TryParse<RetakeStatus>(request.Status, out var status))
            return Result<RetakeResponse>.Fail("Некорректный статус", 400);

        retake.Status = status;
        retake.UpdatedAt = DateTime.UtcNow;
        await db.SaveChangesAsync(ct);
