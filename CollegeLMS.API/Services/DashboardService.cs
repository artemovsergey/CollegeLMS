...
public class DashboardService(AppDbContext db) : IDashboardService
{
    ...
    )
    {
        ...
        var submissions = await db
            .AssignmentSubmissions.Where(s => courseIds.Contains(s.Assignment.CourseId))
        ...
        var studentsCount = await db
            .Students.Where(s => groupIds.Contains(s.GroupId))
            .CountAsync(ct);
    ...
    )
    {
        ...
        var courses = await db.Courses.Where(c => c.GroupId == student.GroupId).ToListAsync(ct);

        var courseIds = courses.Select(c => c.Id).ToList();

        var submissions = await db
            .AssignmentSubmissions.Where(s => s.StudentId == student.Id && s.Score != null)
            .Include(s => s.Assignment)
        ...
        var upcomingDeadlines = await db
            .Assignments.Where(a => courseIds.Contains(a.CourseId) && a.DueDate > DateTime.UtcNow)
            .OrderBy(a => a.DueDate)
            .Take(5)
            .ToListAsync(ct);
