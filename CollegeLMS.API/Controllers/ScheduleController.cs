...
) : ControllerBase
{
    ...
    /// <remarks>
    /// Значения <c>view</c>:
    /// - `day` — день по `date` (по умолчанию сегодня) со слоями: события, практики, пары с бейджами;
    ...
    /// - `week` — неделя Пн–Сб по `week` или `date` (по умолчанию текущая);
    /// - `semester` — матрица семестра; требуется ровно один из `groupId`/`teacherId`, иначе 400;
    /// - без `scope` — легаси-экспорт всего расписания по фильтрам (бот, мини-апп);
    ///   имя файла «Расписание_dd.MM.yyyy_HH-mm-ss».
    ///
