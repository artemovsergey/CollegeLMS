...
public static class DataSeeder
{
    ...
    private static async Task SeedCoursesAsync(AppDbContext db)
    {
        ...
        {
            if (!await db.Courses.AnyAsync(c => c.Title == course.Title))
    ...
    private static async Task SeedReactCourseAsync(AppDbContext db)
    {
        ...
        var teacher = await db
            ...
            .FirstAsync(t => t.User!.Email == "smirnova@collegelms.ru");
        var group31 = await db.Groups.FirstAsync(g => g.Name == "ИСП-31");
        var group32 = await db.Groups.FirstAsync(g => g.Name == "ИСП-32");
        ...
        var course = new Course
        {
            ...
            Description =
                "МДК 01.05 — React: компоненты, хуки, роутинг, Redux, тестирование, Next.js",
        ...
        var assignmentsData = new (string Title, string Description, int MaxScore)[]
        {
            (
                ...
                "Создать React-приложение через Vite. Настроить ESLint, Prettier. Запустить dev-сервер. Создать компонент App с приветствием.",
                10
            ),
            (
                "JSX-компонент ProfileCard",
                "Создать компонент ProfileCard, который принимает пропсы name, role, avatarUrl и рендерит карточку пользователя с использованием JSX.",
                15
            ),
            (
                "Композиция компонентов Layout",
                "Создать компоненты Header, Sidebar, Main, Footer. Скомпоновать их в Layout."
                    + " Header принимает title, Sidebar — items[], Main — children.",
                20
            ),
            (
                "Счётчик с разными шагами",
                "Создать счётчик с кнопками +1, -1, +5, -5, сброс. Отображать историю последних 5 значений. Использовать useState.",
                20
            ),
            (
                "Список задач (Todo)",
                "Реализовать Todo-приложение: добавление, удаление, отметка выполнения, фильтрация (все/активные/выполненные).",
                30
            ),
            (
                "Список задач с React Router",
                "Добавить в Todo-приложение страницы: / (список задач), /stats (статистика), /about. Использовать React Router.",
                25
            ),
            (
                "Форма регистрации с валидацией",
                "Создать форму регистрации: имя, email, пароль, подтверждение пароля. Валидация на стороне клиента (React Hook Form + Yup).",
                25
            ),
            (
                "Форма с динамическими полями",
                "Форма создания опроса: динамические поля вопросов, варианты ответов, тип вопроса (single/multiple).",
                25
            ),
            (
                "Таймер с управлением",
                "Создать таймер обратного отсчёта: ввод времени, старт, пауза, сброс. Использовать useEffect и useRef.",
                20
            ),
            (
                "Погодное приложение",
                "Приложение погоды: поиск города, отображение температуры, влажности, ветра. Использовать OpenWeatherMap API.",
                30
            ),
            (
                "Поиск пользователей GitHub",
                "Поиск пользователей GitHub по логину. Отображать аватар, имя, количество репозиториев. Использовать GitHub API.",
                25
            ),
            (
                "Кастомный хук useLocalStorage",
                "Реализовать хук useLocalStorage, который синхронизирует состояние с localStorage. Использовать в Todo-приложении.",
                20
            ),
            (
                "Кастомный хук useDebounce",
                "Реализовать хук useDebounce для поиска с задержкой. Применить в поиске GitHub-пользователей.",
                20
            ),
            (
                "Контекст темы (светлая/тёмная)",
                "Создать ThemeContext с Provider. Кнопка переключения темы. Применить тёмную тему через CSS-переменные.",
                20
            ),
            (
                "Корзина интернет-магазина",
                "Создать корзину товаров с useReducer: добавление, удаление, изменение количества, подсчёт суммы.",
                30
            ),
            (
                "Список товаров с пагинацией",
                "Список товаров из API (dummyjson) с пагинацией, загрузкой, обработкой ошибок.",
                25
            ),
            (
                "Бесконечная лента (infinite scroll)",
                "Реализовать бесконечную ленту постов с Intersection Observer и загрузкой страниц.",
                25
            ),
            (
                "Модальное окно с порталом",
                "Создать компонент Modal с ReactDOM.createPortal. Клавиша Escape закрывает модалку.",
                20
            ),
            (
                "Аккордеон/FAQ",
                "Создать аккордеон: раскрытие/закрытие секций, возможность открыть несколько или одну.",
                15
            ),
            (
                "Вкладки (Tabs)",
                "Создать компонент Tabs: переключение контента по вкладкам, анимация переключения.",
                15
            ),
            (
                "Redux: список постов",
                "Подключить Redux Toolkit. Создать slice для постов: fetchPosts через createAsyncThunk, отображение списка.",
                25
            ),
            (
                "Redux: корзина и избранное",
                "Создать два slice: cart и favourites. Товары добавляются в корзину и избранное, состояние глобальное.",
                30
            ),
            (
                "Тестирование компонентов",
                "Написать тесты для компонентов ProfileCard, TodoList, Counter с Jest и React Testing Library.",
                25
            ),
            (
                "Тестирование Redux slice",
                "Написать unit-тесты для createSlice и createAsyncThunk: начальное состояние, редьюсеры, middleware.",
                20
            ),
            (
                "Lazy loading страниц",
                "Разделить приложение на lazy-загружаемые страницы через React.lazy и Suspense.",
                15
            ),
            (
                "Скелетоны загрузки",
                "Реализовать компонент Skeleton для состояний загрузки. Анимировать shimmer-эффект.",
                15
            ),
            (
                "Файловый менеджер",
                "Создать компонент файлового менеджера: отображение папок/файлов, навигация, загрузка файлов.",
                30
            ),
            (
                "Чат-компонент",
                "Реализовать простой чат: отправка сообщений, отображение сообщений собеседника, WebSocket-имитация.",
                30
            ),
            (
                "Графики и аналитика",
                "Интегрировать Recharts: график посещаемости, круговая диаграмма распределения, линейный график прогресса.",
                25
            ),
            (
                "Drag and Drop (dnd-kit)",
                "Реализовать перетаскивание элементов списка с @dnd-kit/core. Переупорядочивание задач.",
                25
            ),
            (
                "Календарь событий",
                "Создать календарь: отображение месяца, переключение, добавление/удаление событий.",
                30
            ),
            (
                "Next.js: серверный рендеринг",
                "Перенести приложение на Next.js App Router. Использовать серверные компоненты, layout, SSR.",
                25
            ),
            (
                "Next.js: API routes и middleware",
                "Реализовать Next.js API routes для CRUD. Middleware для аутентификации. ISR для страницы постов.",
                25
            ),
        };
        ...
        )[]
        {
            ...
            (
                "Компоненты и пропсы",
                "Функциональные компоненты, пропсы, композиция",
                10,
                60,
                TestType.SelfStudy
            ),
            (
                "useState и состояние",
                "Хук состояния, подъём состояния, правило хуков",
                10,
                60,
                TestType.SelfStudy
            ),
            (
                "Обработка событий",
                "Синтетические события, обработчики, аргументы",
                10,
                60,
                TestType.SelfStudy
            ),
            ...
            (
                "Формы в React",
                "Управляемые компоненты, select, checkbox",
                10,
                60,
                TestType.SelfStudy
            ),
            ...
            (
                ...
                "useParams, useNavigate, query params",
                10,
                60,
                TestType.SelfStudy
            ),
            ...
            (
                "React Hook Form",
                "Управление формой, валидация, Yup/Zod",
                10,
                60,
                TestType.SelfStudy
            ),
            ...
            (
                "Next.js основы",
                "App Router, SSR, SSG, серверные компоненты",
                15,
                70,
                TestType.Control
            ),
        ...
        var savedTests = await db
            .Tests.Where(t => t.CourseId == course.Id)
            .OrderBy(t => t.Title)
            .ToListAsync();
        ...
        AddQ(
            ...
            [
                "Объектная модель документа браузера",
        ...
        AddQ(
            "Какая команда создаёт React-приложение через Vite?",
            [
        ...
        AddQ(
            "Что возвращает функция-компонент React?",
            ["Объект JavaScript", "Строку HTML", "React-элемент (JSX)", "Виртуальный DOM-узел"],
            "React-элемент (JSX)"
        );
        AddQ(
            "Какой файл является точкой входа в CRA-приложении?",
            ["index.html", "App.js", "main.jsx", "index.js"],
            "index.js"
        );
        AddQ(
            "Какая технология позволяет писать HTML-подобный код внутри JavaScript?",
            ["TypeScript", "Babel", "JSX", "ESLint"],
            "JSX"
        );
        AddQ(
            "Что такое React?",
        ...
        AddQ(
            "Какой хук используется для обновления состояния компонента?",
            ["useEffect", "useContext", "useState", "useReducer"],
            "useState"
        );
        AddQ(
            "Что делает Babel в React-проекте?",
            [
                "Управляет зависимостями",
                "Транспилирует JSX в JavaScript",
                "Собирает финальный бандл",
                "Запускает тесты",
            ],
            "Транспилирует JSX в JavaScript"
        );
        AddQ(
            "Какой пакет отвечает за рендеринг React в браузере?",
            ["react", "react-dom", "react-router", "react-scripts"],
            "react-dom"
        );
        AddQ(
            "Как React обновляет реальный DOM?",
            [
                "Полной перезагрузкой страницы",
                "Сравнением виртуального DOM с реальным (diffing)",
                "Ручным изменением элементов",
                "Через jQuery",
            ],
            "Сравнением виртуального DOM с реальным (diffing)"
        );
        ...
        AddQ(
            ...
            ["Через props", "Через state", "Через ref", "Через контекст"],
            "Через props"
        );
        AddQ(
        ...
        AddQ(
            ...
            ["defaultProps", "initialProps", "static defaultProps = {}", "defaultValue"],
            "static defaultProps = {}"
        );
        AddQ(
            "Что такое композиция компонентов?",
            [
                "Объединение нескольких компонентов в один файл",
                "Вложение одного компонента в другой через children",
        ...
        AddQ(
            "Как получить children в функциональном компоненте?",
            ["this.props.children", "props.children", "children()", "useChildren()"],
            "props.children"
        );
        AddQ(
        ...
        AddQ(
            "Как пробросить props через несколько уровней вложенности?",
            [
                "Через props drilling",
                "Через useContext",
                "Через PropProvider",
                "Через prop forwarding",
            ],
            "Через useContext"
        );
        AddQ(
            "Как рендерить компонент внутри JSX?",
        ...
        // Test 3: useState
        AddQ(
            "Какой хук даёт возможность использовать состояние в функциональном компоненте?",
            ["useEffect", "useContext", "useState", "useRef"],
            "useState"
        );
        AddQ(
            "Что возвращает useState?",
            ["Объект", "Массив из двух элементов", "Число", "Функцию"],
            "Массив из двух элементов"
        );
        AddQ(
            "Как правильно обновить состояние на основе предыдущего?",
            [
                "setCount(count + 1)",
        ...
        AddQ(
            "Что произойдёт при вызове setState с тем же значением?",
            [
                "Компонент перерендерится",
                "React пропустит рендер",
                "Будет ошибка",
                "Сбросится состояние",
            ],
            "React пропустит рендер"
        ...
        AddQ(
            ...
            ["Внутри циклов", "На верхнем уровне компонента", "Внутри условий", "В колбэках"],
        ...
        AddQ(
            ...
            [
                "Удаление состояния",
                "Перенос общего состояния в родительский компонент",
        ...
        AddQ(
            "Сколько раз выполнится setCount(count + 1) при трёх последовательных вызовах?",
            ["1", "3", "0", "Зависит от значения"],
            "3"
        );
        AddQ(
            "Как инициализировать состояние значением из localStorage?",
            [
                "useState(localStorage.getItem('key'))",
                "useState(() => localStorage.getItem('key'))",
                "useEffect(() => localStorage.getItem('key'))",
                "useRef(localStorage.getItem('key'))",
            ],
            "useState(() => localStorage.getItem('key'))"
        );
        AddQ(
            "Как объявить несколько состояний в одном компоненте?",
            [
                "Вызвать useState несколько раз",
        ...
        AddQ(
            "Какое правило НЕ относится к правилам хуков?",
            [
                "Хуки вызываются на верхнем уровне",
                "Хуки вызываются только из React-функций",
                "Хуки можно вызывать внутри условий",
        ...
        // Test 4: События
        AddQ(
            "Как React называет свою систему событий?",
            ["DOM Events", "Synthetic Events", "Virtual Events", "React Events"],
            "Synthetic Events"
        );
        AddQ(
            "Как передать аргумент в обработчик события?",
            [
                "onClick={handleClick(arg)}",
                "onClick={() => handleClick(arg)}",
                "onClick={handleClick.bind(arg)}",
                "onClick={handleClick} arg={arg}",
            ],
            "onClick={() => handleClick(arg)}"
        );
        AddQ(
            "Как предотвратить действие по умолчанию в React?",
            [
        ...
        AddQ(
            "Какой атрибут используется для события клика?",
            ["onclick", "onClick", "on-click", "OnClick"],
            "onClick"
        );
        AddQ(
            "Как получить значение поля ввода при onChange?",
            ["e.value", "e.target.value", "e.current.value", "e.data"],
            "e.target.value"
        );
        AddQ(
            "SyntheticEvent в React — это:",
            [
                "Нативный браузерный объект",
                "Кроссплатформенная обёртка над нативным событием",
        ...
        AddQ(
            "Как передать id элемента в onClick?",
            ["data-id атрибут", "Замыкание с параметром", "dataset свойство", "id пропс"],
            "data-id атрибут",
            QuestionType.MultipleChoice
        );
        AddQ(
            "Что произойдёт без вызова stopPropagation?",
            ["Событие всплывёт к родителям", "Событие отменится", "Форма отправится", "Ничего"],
            "Событие всплывёт к родителям"
        );
        AddQ(
            "Как подписаться на событие клавиатуры?",
            ["onKeyDown", "onKeyboard", "onPress", "onType"],
            "onKeyDown"
        );
        AddQ(
            "Что такое event pooling в React?",
            [
                "События хранятся в пуле и переиспользуются",
                "События удаляются после обработки",
                "События ставятся в очередь",
                "События кэшируются",
            ],
            "События хранятся в пуле и переиспользуются"
        );
        ...
        AddQ(
            "Какой оператор НЕ используется для условного рендеринга?",
            ["&&", "?", "||", "if/else"],
            "||"
        );
        AddQ(
            "Как отрендерить список элементов из массива?",
            ["list.forEach()", "list.map()", "list.filter()", "list.reduce()"],
            "list.map()"
        );
        AddQ(
            ...
            [
                "Для стилизации",
                "Для идентификации элементов при обновлении",
                "Для сортировки",
        ...
        AddQ(
            "Что будет, если не указать key в списке?",
            [
                "Ошибка компиляции",
                "React выдаст предупреждение, производительность упадёт",
                "Ничего",
                "Список не отрендерится",
            ],
            "React выдаст предупреждение, производительность упадёт"
        );
        AddQ(
            "Какое значение key считается лучшей практикой?",
            ["Индекс в массиве", "Уникальный id элемента", "Случайное число", "Имя элемента"],
            "Уникальный id элемента"
        );
        AddQ(
            "Что вернёт выражение {isLogged && <LogoutButton />} если isLogged = false?",
            ["false", "null", "LogoutButton", "Ошибку"],
            "false"
        );
        AddQ(
            "Как отфильтровать массив перед рендерингом?",
            ["filter().map()", "map().filter()", "forEach().filter()", "reduce().filter()"],
            "filter().map()"
        );
        AddQ(
            "Как отрендерить элементы по индексу?",
            [
                "Добавить key={index}",
                "Использовать i < items.length",
                "Применить Object.keys()",
                "Использовать for",
            ],
            "Добавить key={index}"
        );
        AddQ(
            "Что такое условный return в компоненте?",
            [
                "Возврат null если условие не выполнено",
                "Двойной return",
                "Тернарный return",
                "Асинхронный return",
            ],
        ...
        AddQ(
            "Как отрендерить несколько элементов без обёртки?",
            ["<></> или Fragment", "Вернуть массив", "Обернуть в div", "Использовать span"],
            "<></> или Fragment"
        );

        // Test 6: Формы
        AddQ(
            "Что такое управляемый компонент формы?",
            [
                "Компонент без состояния",
                "Компонент, чьё значение контролируется React через state",
                "Компонент с ref",
                "HTML-элемент form",
            ],
            "Компонент, чьё значение контролируется React через state"
        ...
        AddQ(
            "Как получить значение select в управляемом компоненте?",
            [
        ...
        AddQ(
            ...
            [
                "Компонент без onChange",
                "Компонент, где значение хранится в DOM",
                "Компонент без state",
                "Пустой компонент",
            ],
            "Компонент, где значение хранится в DOM"
        ...
        AddQ(
            "Как создать ref для неуправляемого поля?",
            ["useRef() + ref атрибут", "createRef()", "useState()", "useEffect()"],
            "useRef() + ref атрибут"
        );
        AddQ(
            "Как обработать отправку формы?",
            ["onClick на кнопке", "onSubmit на форме", "onChange на форме", "onSubmit на кнопке"],
            "onSubmit на форме"
        );
        AddQ(
            "Как сбросить форму после отправки?",
            ["form.reset()", "setState с пустыми значениями", "reload()", "clearForm()"],
            "setState с пустыми значениями"
        );
        AddQ(
            "Как реализовать checkbox в управляемой форме?",
            [
                "checked={isChecked} onChange={handler}",
                "value={isChecked}",
                "defaultChecked",
                "ref={checkboxRef}",
            ],
        ...
        AddQ(
            "Как получить все значения формы сразу при submit?",
            [
                "FormData(e.target)",
                "e.target.value",
        ...
        AddQ(
            "Можно ли использовать defaultValue в управляемых полях?",
            ["Да", "Нет, это бессмысленно", "Только в select", "Только в textarea"],
        ...
        AddQ(
            ...
            ["Для рендеринга", "Для побочных эффектов", "Для управления состоянием", "Для стилей"],
            "Для побочных эффектов"
        );
        AddQ(
            "Что произойдёт при пустом массиве зависимостей []?",
            [
                "Эффект выполнится при каждом рендере",
                "Эффект выполнится только при монтировании",
                "Эффект не выполнится",
                "Будет ошибка",
            ],
            "Эффект выполнится только при монтировании"
        );
        AddQ(
            "Как очистить подписку в useEffect?",
            [
        ...
        AddQ(
            "Когда выполняется эффект с зависимостями [a, b]?",
            [
                "При каждом рендере",
                "Только при изменении a или b",
                "При монтировании и изменении a или b",
                "Один раз",
            ],
            "При монтировании и изменении a или b"
        );
        AddQ(
            "Как сделать HTTP-запрос при монтировании?",
            [
                "fetch внутри useEffect с []",
                "fetch на верхнем уровне",
                "fetch в useState",
                "fetch в useRef",
            ],
            "fetch внутри useEffect с []"
        ...
        AddQ(
            "Как избежать бесконечного цикла при fetch внутри useEffect?",
            [
                "Добавить пустой массив зависимостей",
        ...
        AddQ(
            "Что если не указать массив зависимостей вообще?",
            [
        ...
        AddQ(
            ...
            ["AbortController + signal", "CancelToken", "Отмена через useEffect cleanup", "Никак"],
            "AbortController + signal"
        );
        AddQ(
            "Можно ли использовать async функцию в useEffect напрямую?",
            ["Да", "Нет, нужно обернуть", "Только с useCallback", "Только с then"],
            "Нет, нужно обернуть"
        );
        AddQ(
            "Как симулировать componentDidMount с useEffect?",
            [
                "useEffect(() => {}, [])",
                "useEffect(() => {})",
                "useEffect(() => {}, null)",
        ...
        AddQ(
            ...
            ["{ current: initialValue }", "initialValue", "Объект", "Функцию"],
            "{ current: initialValue }"
        );
        AddQ(
            "Изменение .current у useRef вызывает ли перерендер?",
            ["Да", "Нет", "Зависит от значения", "Только при первом изменении"],
            "Нет"
        );
        AddQ(
            "Как получить DOM-элемент по ref?",
            ["ref.current", "ref.value", "current(ref)", "ref.dom"],
            "ref.current"
        );
        AddQ(
            "Для чего используется forwardRef?",
            [
                "Для рефакторинга",
                "Для передачи ref дочернему компоненту",
                "Для создания ref",
                "Для проброса props",
            ],
            "Для передачи ref дочернему компоненту"
        ...
        AddQ(
            "Как поставить фокус на поле ввода при монтировании?",
            [
        ...
        AddQ(
            "Можно ли использовать useRef для хранения мутируемого значения?",
            [
                "Да, это его основное назначение кроме DOM",
                "Нет, только для DOM",
                "Только для чисел",
                "Только с useState",
            ],
        ...
        AddQ(
            "Как измерить ширину элемента после рендера?",
            [
                "useRef + offsetWidth в useEffect",
        ...
        AddQ(
            "Что произойдёт при попытке доступа к ref.current до монтирования?",
            ["null", "undefined", "Ошибка", "Пустой объект"],
            "null"
        );
        AddQ(
            "Как передать ref через компонент-обёртку?",
            [
                "forwardRef + второй аргумент",
                "prop с именем ref",
                "useImperativeHandle",
                "cloneElement",
            ],
        ...
        AddQ(
            ...
            ["React.createContext()", "useContext()", "createContext()", "new Context()"],
            "React.createContext()"
        );
        AddQ(
            "Что возвращает createContext?",
            ["Объект с Provider и Consumer", "Компонент-обёртку", "Хук", "Строку"],
            "Объект с Provider и Consumer"
        );
        AddQ(
            "Какой компонент предоставляет значение контекста?",
            ["Context.Provider", "Context.Consumer", "Context.Provider value={}", "Context"],
            "Context.Provider value={}"
        );
        AddQ(
            "Как получить значение контекста в функциональном компоненте?",
            ["useContext(Context)", "Context.Consumer", "this.context", "useContext()"],
            "useContext(Context)"
        );
        AddQ(
            "Когда React перерендеривает потребителей контекста?",
            ["При изменении значения Provider", "При любом рендере", "Никогда", "По таймеру"],
            "При изменении значения Provider"
        );
        AddQ(
            "Что произойдёт при изменении state родителя, содержащего Provider?",
            [
                "Все потребители перерендерятся",
                "Provider не перерендерит потребителей",
                "Перерендерятся только дети Provider",
                "Ничего",
            ],
            "Все потребители перерендерятся"
        );
        AddQ(
            "Как оптимизировать контекст во избежание лишних рендеров?",
            [
        ...
        AddQ(
            "Можно ли вкладывать Provider одного контекста друг в друга?",
            ["Да", "Нет", "Только два уровня", "С ограничениями"],
            "Да"
        );
        AddQ(
            "Что делает Context.Consumer?",
            [
                "Рендерит children как функцию с value",
        ...
        AddQ(
            "Контекст следует использовать для:",
            [
                "Глобальных данных (тема, язык, auth)",
                "Каждого пропса",
                "Только для состояния",
                "Только для стилей",
            ],
            "Глобальных данных (тема, язык, auth)"
        );
        ...
        // Test 10: useReducer
        AddQ(
            "Из каких частей состоит useReducer?",
            [
        ...
        AddQ(
            ...
            ["[state, dispatch]", "[state, reducer]", "{state, dispatch}", "[reducer, dispatch]"],
            "[state, dispatch]"
        );
        AddQ(
            "Что такое action в useReducer?",
            ["Объект с полем type и опционально payload", "Функция", "Строка", "Число"],
            "Объект с полем type и опционально payload"
        );
        AddQ(
            "Как dispatch отправляет action?",
            [
        ...
        AddQ(
            "Где должна быть чистая логика редьюсера?",
            ["Внутри функции-редьюсера", "В dispatch", "В action", "В компоненте"],
            "Внутри функции-редьюсера"
        );
        AddQ(
            "Когда использовать useReducer вместо useState?",
            [
                "Когда состояние сложное или зависит от предыдущего",
                "Всегда",
                "Никогда",
                "Только с TypeScript",
            ],
        ...
        AddQ(
            "Как редьюсер должен обрабатывать неизвестный action type?",
            ["Вернуть текущее состояние", "Выбросить ошибку", "Вернуть undefined", "Пропустить"],
            "Вернуть текущее состояние"
        );
        AddQ(
            "Можно ли делать асинхронные вызовы внутри редьюсера?",
            [
                "Нет, редьюсер должен быть чистым",
                "Да, напрямую",
                "Да, через dispatch",
                "Только через thunk",
            ],
        ...
        AddQ(
            "Что такое initializer для useReducer?",
            [
                "Третий аргумент — функция ленивой инициализации",
                "Начальное состояние",
                "Диспатчер",
                "Action creator",
            ],
        ...
        AddQ(
            "Как смоделировать enum action types в TypeScript?",
            ["Строковый union тип", "Числовые константы", "Enum", "Interface"],
            "Строковый union тип"
        );

        // Test 11: Кастомные хуки
        AddQ(
            "Какие правила нужно соблюдать при создании кастомных хуков?",
            [
                "Начинать с 'use', вызывать другие хуки",
                "Заканчивать на 'Hook'",
                "Использовать классы",
                "Импортировать из React",
            ],
            "Начинать с 'use', вызывать другие хуки"
        );
        AddQ(
            "Кастомный хук может вызывать другие хуки?",
            ["Да", "Нет", "Только useState", "Только useEffect"],
            "Да"
        );
        AddQ(
            "Что типизирует возвращаемое значение кастомного хука?",
            ["Кортеж или объект", "Только число", "Только строку", "undefined"],
            "Кортеж или объект"
        );
        AddQ(
            "Как хук useLocalStorage должен синхронизировать данные?",
            ["useState + useEffect для записи в localStorage", "useRef", "useReducer", "useMemo"],
            "useState + useEffect для записи в localStorage"
        );
        AddQ(
            "Можно ли использовать кастомный хук внутри условия?",
            ["Нет, нарушает правила хуков", "Да", "Только если хук простой", "С dependsOn"],
            "Нет, нарушает правила хуков"
        );
        AddQ(
            "Как передать параметры в кастомный хук?",
            ["Как аргументы функции", "Через контекст", "Через props", "Через ref"],
            "Как аргументы функции"
        );
        AddQ(
            "Какой хук использовать для дебаунса ввода?",
            [
                "useDebounce (кастомный)",
                "useEffect с setTimeout",
                "onChange с setTimeout",
                "useMemo",
            ],
            "useDebounce (кастомный)"
        );
        AddQ(
            "Для чего используют useToggle?",
            ["Для переключения boolean состояния", "Для счётчика", "Для запросов", "Для анимаций"],
            "Для переключения boolean состояния"
        );
        AddQ(
            "Что вернёт хук useFetch(url)?",
            ["{ data, loading, error }", "{ data }", "data", "Promise"],
            "{ data, loading, error }"
        );
        AddQ(
            "Как композировать хуки?",
            [
                "Вызвать один кастомный хук внутри другого",
                "Импортировать вложенный хук",
                "Наследовать хук",
                "Создать класс",
            ],
            "Вызвать один кастомный хук внутри другого"
        ...
        AddQ(
            "Когда useMemo не даёт выигрыша?",
            [
                "При простых вычислениях",
                "При сложных вычислениях",
                "При частых изменениях зависимостей",
                "Никогда",
            ],
            "При простых вычислениях"
        );
        AddQ(
            "Как React.memo сравнивает props?",
            [
        ...
        AddQ(
            "Как передать функцию сравнения в React.memo?",
            [
                "React.memo(Component, areEqual)",
                "React.memo(Component, compareFn)",
                "React.memo(areEqual)(Component)",
                "React.memo(Component, propsAreEqual)",
            ],
            "React.memo(Component, areEqual)"
        );
        AddQ(
            "Что произойдёт без useCallback при передаче inline функции?",
            [
                "Компонент будет создавать новую функцию на каждый рендер",
                "Ничего",
                "Будет ошибка",
                "Функция закешируется",
            ],
            "Компонент будет создавать новую функцию на каждый рендер"
        );
        AddQ(
            "Какой инструмент помогает обнаружить лишние рендеры?",
            ["React DevTools Profiler", "Chrome DevTools", "ESLint", "TypeScript"],
            "React DevTools Profiler"
        );
        AddQ(
            "Мемоизация нужна:",
            [
                "Только при доказанной проблеме производительности",
                "Всегда",
                "Никогда",
                "Только для классов",
            ],
        ...
        AddQ(
            "Что передать вторым аргументом useCallback, чтобы функция не менялась?",
            ["[]", "null", "undefined", "Зависимости"],
            "[]"
        );

        // Test 13: React Router
        AddQ(
            "Какой компонент оборачивает всё приложение для роутинга?",
            ["BrowserRouter", "Router", "Routes", "Route"],
            "BrowserRouter"
        );
        AddQ(
            "Какой компонент определяет маршрут?",
            ["<Route path='/' element={<Home />} />", "Route", "Router", "Path"],
            "<Route path='/' element={<Home />} />"
        );
        AddQ(
            "Какой компонент рендерит подходящий маршрут?",
            ["<Routes>", "<Route>", "<Switch>", "Router"],
            "<Routes>"
        );
        AddQ(
            "Как перейти на другую страницу без перезагрузки?",
            [
                "<Link to='/about'>",
                "<a href='/about'>",
        ...
        AddQ(
            "Чем NavLink отличается от Link?",
            [
                "NavLink добавляет active class",
                "NavLink быстрее",
                "NavLink не перезагружает страницу",
                "NavLink только для меню",
            ],
            "NavLink добавляет active class"
        );
        AddQ(
            "Как получить текущий pathname?",
            ["useLocation()", "useParams()", "useNavigate()", "usePathname()"],
            "useLocation()"
        );
        AddQ(
            "Как сделать редирект после действия?",
            ["useNavigate()('path')", "redirect('path')", "history.push('path')", "Link to='path'"],
            "useNavigate()('path')"
        );
        AddQ(
            "Как сделать вложенный маршрут в React Router v6?",
            [
                "<Route path='parent'><Route path='child'/></Route>",
                "Вложить Route внутри другого Route",
                "Использовать Outlet",
        ...
        AddQ(
            "Какой элемент рендерит дочерний маршрут?",
            ["<Outlet />", "<Child />", "<RouteChildren />", "<NestedRoute />"],
            "<Outlet />"
        );
        AddQ(
            "Что такое index route?",
            [
                "Путь по умолчанию для родителя",
                "Маршрут с индексом",
                "Служебный маршрут",
                "Главная страница",
            ],
        ...
        AddQ(
            "Как получить параметры из URL?: /users/42",
            ["useParams()", "useLocation()", "useQuery()", "useNavigate()"],
            "useParams()"
        );
        AddQ(
        ...
        AddQ(
            "Как передать state при навигации?",
            [
        ...
        AddQ(
            "Как защитить маршрут от неавторизованных пользователей?",
            ["Создать ProtectedRoute компонент", "Guard middleware", "Route guard", "Auth wrapper"],
            "Создать ProtectedRoute компонент"
        );
        AddQ(
            "Что делает useNavigate при вызове с -1?",
            [
        ...
        AddQ(
            "Как подписаться на изменение location?",
            [
                "useEffect с location как зависимостью",
                "useLocation()",
                "useNavigate()",
                "subscribe()",
            ],
        ...
        AddQ(
            "Как сделать 404 страницу в React Router?",
            [
        ...
        AddQ(
            "Можно ли использовать React Router вне браузера?",
            [
                "Да, есть NativeRouter и StaticRouter",
                "Нет",
                "Только ServerRouter",
                "Только MemoryRouter",
            ],
        ...
        AddQ(
            "Как обработать ответ fetch?",
            ["res.json()", "res.text()", "res.data()", "res.parse()"],
            "res.json()"
        );
        AddQ(
            "Чем отличается axios от fetch?",
            [
                "axios автоматически парсит JSON",
                "fetch быстрее",
                "axios не поддерживает перехватчики",
                "fetch парсит JSON автоматически",
            ],
            "axios автоматически парсит JSON"
        );
        AddQ(
            "Как отменить fetch-запрос?",
            ["AbortController + signal", "cancel()", "abort()", "stop()"],
            "AbortController + signal"
        );
        AddQ(
            "Как обработать ошибку сети в fetch?",
            ["try/catch", ".catch()", "error callback", "onError"],
            "try/catch"
        );
        AddQ(
        ...
        AddQ(
            "Для чего нужен перехватчик (interceptor) в axios?",
            [
                "Для модификации запросов/ответов глобально",
                "Для логирования",
                "Для кэширования",
                "Для трансформации",
            ],
        ...
        AddQ(
            "Как установить заголовок Authorization?",
            [
        ...
        AddQ(
            "Что такое CORS и когда возникает ошибка?",
            [
                "Политика браузера при запросе на другой домен",
                "Ошибка сервера",
                "Проблема с JSON",
                "Ошибка валидации",
            ],
        ...
        AddQ(
            "Как выполнить несколько параллельных запросов?",
            ["Promise.all([fetch1, fetch2])", "fetch.all()", "parallel()", "join()"],
            "Promise.all([fetch1, fetch2])"
        );

        // Test 16: Загрузка и ошибки
        AddQ(
            "Какой компонент ловит ошибки рендеринга?",
            ["ErrorBoundary", "ErrorHandler", "CatchError", "SafeComponent"],
            "ErrorBoundary"
        );
        AddQ(
            "ErrorBoundary ловит ошибки в:",
            [
                "Дочерних компонентах",
                "Своих обработчиках событий",
                "Асинхронном коде",
        ...
        AddQ(
            "Как показать спиннер во время загрузки?",
            [
                "Условный рендеринг на основе loading state",
                "useLoading()",
                "Loading компонент",
                "Spin компонент",
            ],
        ...
        AddQ(
            "Что такое скелетон (skeleton)?",
            [
                "Заглушка загрузки, имитирующая структуру контента",
                "Тип анимации",
                "Пустой компонент",
                "Спиннер",
            ],
        ...
        AddQ(
            "Как показать сообщение об ошибке пользователю?",
            [
                "Error-компонент с текстом ошибки и кнопкой повтора",
        ...
        AddQ(
            "Какой жизненный цикл ошибки в ErrorBoundary?",
            [
        ...
        AddQ(
            "Можно ли ErrorBoundary восстановить UI?",
            [
                "Да, через сброс состояния",
                "Нет, только показать fallback",
                "Автоматически",
                "Через reload",
            ],
            "Да, через сброс состояния"
        ...
        AddQ(
            "Как симулировать задержку при тестировании?",
            ["await new Promise(r => setTimeout(r, ms))", "sleep(ms)", "delay(ms)", "wait(ms)"],
            "await new Promise(r => setTimeout(r, ms))"
        );
        AddQ(
            "Что такое fallback UI в ErrorBoundary?",
            ["Резервный UI при ошибке", "UI по умолчанию", "Загрузочный UI", "Пустой UI"],
            "Резервный UI при ошибке"
        );
        AddQ(
            "Как показать тост с ошибкой?",
            [
                "Библиотека уведомлений (sonner, react-toastify)",
                "alert()",
                "Окно браузера",
                "console.error",
            ],
        ...
        AddQ(
            "Какой хук предоставляет React Hook Form?",
            ["useForm()", "useFormHook()", "useFormState()", "form()"],
            "useForm()"
        );
        AddQ(
            "Как зарегистрировать поле в RHF?",
            [
                "{...register('fieldName')}",
                "register('fieldName')",
                "useRegister('fieldName')",
                "bind('fieldName')",
            ],
            "{...register('fieldName')}"
        );
        AddQ(
            "Как получить ошибки формы в RHF?",
            ["formState.errors", "errors", "formErrors", "validationErrors"],
            "formState.errors"
        );
        AddQ(
            "Как интегрировать Yup/Zod с RHF?",
            ["resolver от @hookform/resolvers", "validationSchema", "schema", "validator"],
            "resolver от @hookform/resolvers"
        );
        AddQ(
            "Как сбросить форму в RHF?",
            ["reset()", "clear()", "resetForm()", "clean()"],
            "reset()"
        );
        AddQ(
            "Как показать сообщение об ошибке под полем?",
            [
        ...
        AddQ(
            "Как отслеживать изменения поля в RHF?",
            ["watch('fieldName')", "onChange", "subscribe('fieldName')", "track('fieldName')"],
            "watch('fieldName')"
        );
        AddQ(
            "Как отправить форму в RHF?",
            [
        ...
        AddQ(
            "Как добавить кастомную валидацию в Yup?",
            [
        ...
        AddQ(
            "Какой тип Zod подходит для строки email?",
            ["z.string().email()", "z.email()", "z.string().email", "z.email().string()"],
            "z.string().email()"
        );
        ...
        AddQ(
            ...
            [
                "Фрагмент состояния с редьюсерами и экшенами",
                "Файл конфигурации",
                "Тип экшена",
                "MiddleWare",
            ],
        ...
        AddQ(
            ...
            [
                "Создаёт Redux store с devtools и middleware",
                "Настраивает роутер",
                "Конфигурирует API",
                "Устанавливает темы",
            ],
        ...
        AddQ(
            ...
            [
                "useSelector(state => state.some)",
                "useStore(state => state.some)",
                "getState().some",
                "useSelect(state => state.some)",
            ],
            "useSelector(state => state.some)"
        );
        AddQ(
            "Как отправить экшен?",
            ["useDispatch()()", "dispatch()", "store.dispatch(action)", "send(action)"],
            "useDispatch()()"
        );
        AddQ(
            "Как создать экшен с payload в slice?",
            [
                "reducers: { myAction: (state, action) => {} }",
                "actions: { myAction }",
                "reducers.myAction = (state, action) => {}",
                "payload: myAction",
            ],
        ...
        AddQ(
            "Можно ли мутировать state в Redux Toolkit?",
            [
                "Да, Immer позволяет",
                "Нет, только иммутабельно",
                "Только через createAction",
                "Через spread",
            ],
            "Да, Immer позволяет"
        );
        AddQ(
            "Как добавить middleware в configureStore?",
            [
        ...
        AddQ(
            ...
            [
                "Редьюсеры для внешних экшенов (createAsyncThunk)",
        ...
        AddQ(
            "Как типизировать useSelector?",
            [
                "(state: RootState) => state.value",
                "useSelector<RootState>('value')",
                "useSelector<ReturnType<typeof store.getState>>(state => state.value)",
                "useSelector typed",
            ],
            "(state: RootState) => state.value"
        );
        ...
        AddQ(
            ...
            ["Три экшена: pending/fulfilled/rejected", "Один экшен", "Slice", "Reducer"],
            "Три экшена: pending/fulfilled/rejected"
        );
        AddQ(
            "Где обрабатываются экшены createAsyncThunk?",
            ["В extraReducers", "В reducers", "В middleware", "В thunks"],
            "В extraReducers"
        );
        AddQ(
            "Как обработать успешный ответ thunk?",
            [
                ".addCase(fetchUsers.fulfilled, (state, action) => {})",
                ".addCase(fetchUsers.done, ...)",
                ".addCase(fetchUsers.ok, ...)",
                ".addCase(fetchUsers.success, ...)",
            ],
            ".addCase(fetchUsers.fulfilled, (state, action) => {})"
        );
        AddQ(
            "Что такое builder в extraReducers?",
            ["Объект для цепочки addCase", "Конструктор", "Функция", "Callback"],
            "Объект для цепочки addCase"
        );
        AddQ(
            "Как типизировать возвращаемое значение thunk?",
            [
                "Тип в дженерике createAsyncThunk<ReturnType, ArgType>",
                "Автоматически",
                "Через ReturnType",
                "Через Promise<Type>",
            ],
        ...
        AddQ(
            "Как передать аргумент в thunk?",
            ["dispatch(thunk(arg))", "thunk.dispatch(arg)", "thunk(arg)", "thunk.arg()"],
            "dispatch(thunk(arg))",
            QuestionType.MultipleChoice
        );
        AddQ(
            "Как обработать ошибку в thunk?",
            [
                "return rejectWithValue(error)",
                "throw error",
                "return thunk.reject(error)",
                "return { error }",
            ],
            "return rejectWithValue(error)"
        );
        AddQ(
            "Где лучше делать асинхронные запросы в Redux?",
            ["В createAsyncThunk", "В редьюсере", "В компоненте useEffect", "В middleware"],
            "В createAsyncThunk"
        );
        AddQ(
            "Как обновить состояние после rejected?",
            [
                ".addCase(thunk.rejected, (state, action) => { state.error = action.payload })",
                "Ошибка обрабатывается автоматически",
                "В extraReducers.rejected",
                "В rejected",
            ],
            ".addCase(thunk.rejected, (state, action) => { state.error = action.payload })"
        );
        AddQ(
            "Что такое matcher в extraReducers?",
            ["Функция для группировки нескольких экшенов", "Утилита", "Тип", "Middleware"],
            "Функция для группировки нескольких экшенов"
        );

        // Test 20: Тестирование
        AddQ(
            "Какая библиотека используется для тестирования React-компонентов?",
            ["@testing-library/react", "Jest", "Mocha", "Chai"],
            "@testing-library/react"
        );
        AddQ(
            "Что делает render из RTL?",
            [
                "Рендерит компонент в jsdom",
                "Рендерит на сервере",
        ...
        AddQ(
            "Как найти элемент по роли?",
            [
                "screen.getByRole('button')",
                "findByRole('button')",
                "getElementByRole('button')",
                "getByRole('button')",
            ],
            "screen.getByRole('button')"
        );
        AddQ(
            "Что делает userEvent?",
            [
                "Симулирует более реалистичные события, чем fireEvent",
                "Событие мыши",
                "Событие клавиатуры",
                "Ивент",
            ],
        ...
        AddQ(
            "Как проверить, что элемент не отрендерился?",
            [
                "expect(screen.queryByText('text')).toBeNull()",
                "expect.not(screen.getByText('text'))",
                "screen.assertNoText('text')",
                "expect(screen.getByText('text')).not.toBeInTheDocument()",
            ],
            "expect(screen.queryByText('text')).toBeNull()"
        );
        AddQ(
            "Как обернуть компонент в Provider для теста?",
            [
                "render(<Provider><Component /></Provider>)",
                "wrapInProvider(Component)",
                "Provider.render(Component)",
                "mockProvider(Component)",
            ],
            "render(<Provider><Component /></Provider>)"
        );
        AddQ(
            "Как протестировать хук?",
            [
        ...
        AddQ(
            "Как проверить, что функция была вызвана с определёнными аргументами?",
            [
                "expect(mock).toHaveBeenCalledWith(arg)",
                "mock.calls.includes(arg)",
                "assertCalled(mock, arg)",
                "verify(mock, arg)",
            ],
            "expect(mock).toHaveBeenCalledWith(arg)"
        );
        ...
        // Test 21: Оптимизация
        AddQ(
            "Как лениво загрузить компонент?",
            [
        ...
        AddQ(
            "Что рендерит Suspense пока грузится lazy-компонент?",
            ["fallback пропс", "spinner", "null", "loading"],
            "fallback пропс"
        );
        AddQ(
            "Что такое code splitting?",
            ["Разделение кода на чанки", "Удаление мёртвого кода", "Минификация", "Обфускация"],
            "Разделение кода на чанки"
        );
        AddQ(
            "Как использовать React.lazy с роутингом?",
            [
                "lazy(() => import('./Pages/Home')) внутри Route element",
                "Только на главной",
                "Вне BrowserRouter",
                "В index.js",
            ],
        ...
        AddQ(
            ...
            [
                "Производительность рендера компонента",
                "Размер бандла",
                "Скорость сети",
                "Потребление памяти",
            ],
        ...
        AddQ(
            "Как избежать re-render при изменении пропсов родителя?",
            [
        ...
        AddQ(
            "Какая библиотека помогает с виртуализацией?",
            ["react-window или react-virtuoso", "react-list", "virtual-list", "react-virtualized"],
            "react-window или react-virtuoso"
        );
        AddQ(
            "Что такое bundle-анализ?",
            [
                "Проверка размера бандла (webpack-bundle-analyzer)",
                "Анализ кода",
                "Проверка ошибок",
                "Линтинг",
            ],
        ...
        AddQ(
            ...
            ["Tree shaking (dead code elimination)", "Минификация", "Uglify", "Обфускация"],
            "Tree shaking (dead code elimination)"
        );

        // Test 22: Сборка и деплой
        AddQ(
            "Какая команда собирает production-бандл Vite?",
            ["vite build", "npm run build", "vite production", "npm run prod"],
            "vite build"
        );
        AddQ(
            "Что делает Docker в контексте React?",
            ["Упаковывает приложение с nginx", "Компилирует код", "Запускает тесты", "Дебажит"],
            "Упаковывает приложение с nginx"
        );
        AddQ(
            "Как настроить переменные окружения в Vite?",
            ["VITE_API_URL в .env файле", "REACT_APP_API_URL", "NEXT_PUBLIC_API_URL", "API_URL"],
            "VITE_API_URL в .env файле"
        );
        AddQ(
            "Что делает nginx в production-сборке React?",
            [
                "Раздаёт статику и проксирует API",
                "Компилирует React",
                "Сервер рендеринга",
                "База данных",
            ],
        ...
        AddQ(
            ...
            [
                "Подключить Git-репозиторий → авто-деплой",
                "Загрузить ZIP",
                "Через FTP",
                "SCP файлов",
            ],
        ...
        AddQ(
            "Что такое CI/CD?",
            ["Непрерывная интеграция и доставка", "Компиляция", "Тестирование", "Документация"],
            "Непрерывная интеграция и доставка"
        );
        AddQ(
            "Как сделать preview-деплой для PR?",
            [
        ...
        AddQ(
            "Как кэшировать node_modules в Docker?",
            [
        ...
        AddQ(
            "Что выдает vite build при успешной сборке?",
            ["Папку dist со статическими файлами", "server.js", "bundle.js", "index.html"],
            "Папку dist со статическими файлами"
        );
        AddQ(
            "Как проверить размер бандла после сборки?",
            ["vite build --analyze", "ls -la dist", "npm run analyze", "webpack-bundle-analyzer"],
            "vite build --analyze"
        );
        ...
        AddQ(
            ...
            [
                "SSR рендерит на сервере, CSR в браузере",
                "Только названием",
                "SSR быстрее CSR",
                "CSR безопаснее",
            ],
        ...
        AddQ(
            "Что такое App Router в Next.js 13+?",
            [
                "Новый маршрутизатор на основе файловой системы",
                "Клиентский роутер",
                "Серверный роутер",
                "Роутер для API",
            ],
        ...
        AddQ(
            "Как создать серверный компонент в Next.js?",
            [
                "Просто export default function в app/",
                "Добавить 'use server'",
                "Добавить 'use client'",
        ...
        AddQ(
            "Как сделать страницу динамической (SSR)?",
            [
        ...
        AddQ(
            "Как загружать данные в серверном компоненте?",
            ["async компонент с await fetch()", "useEffect", "getServerSideProps", "useSWR"],
            "async компонент с await fetch()"
        );
        AddQ(
            "Что такое layout в Next.js?",
            ["Обёртка для дочерних страниц", "Стиль страницы", "Шаблон", "Компонент"],
            "Обёртка для дочерних страниц"
        );
        AddQ(
            "Как создать клиентский компонент в Next.js?",
            [
        ...
        AddQ(
            "Как настроить ISR в Next.js?",
            ["export const revalidate = 60", "revalidate: 60", "isr: true", "cache: 'force-cache'"],
            "export const revalidate = 60"
        );

        // Test 24: Next.js роутинг
        AddQ(
            "Как создать API route в Next.js App Router?",
            [
                "app/api/route.ts с export async function GET()",
                "api/route.js",
                "pages/api/route.js",
                "server/api.ts",
            ],
        ...
        AddQ(
            "Как создать динамический маршрут [id]?",
            [
                "app/users/[id]/page.tsx",
                "app/users/:id/page.tsx",
                "app/users/_id/page.tsx",
                "app/users/$id/page.tsx",
            ],
            "app/users/[id]/page.tsx"
        );
        AddQ(
            ...
            ["Функция, выполняемая перед запросом", "Компонент", "Плагин", "Конфиг"],
            "Функция, выполняемая перед запросом"
        );
        AddQ(
            "Как редиректить в Next.js middleware?",
            [
        ...
        AddQ(
            "Где лежит файл middleware в Next.js?",
            [
        ...
        AddQ(
            "Как получить searchParams в серверном компоненте?",
            [
                "props.searchParams",
                "useSearchParams()",
                "getSearchParams()",
                "searchParams из контекста",
            ],
            "props.searchParams"
        );
        AddQ(
            "Как сделать catch-all маршрут [...slug]?",
            [
                "app/shop/[...slug]/page.tsx",
                "app/shop/*/page.tsx",
                "app/shop/__slug__/page.tsx",
                "app/shop/:slug/page.tsx",
            ],
            "app/shop/[...slug]/page.tsx"
        );
        AddQ(
            "Как генерировать статические параметры?",
            [
                "export async function generateStaticParams()",
                "getStaticPaths()",
                "generateParams()",
        ...
        AddQ(
            "Что делает notFound() в Next.js?",
            ["Рендерит страницу 404", "Редиректит на 404", "Бросает ошибку", "Возвращает null"],
            "Рендерит страницу 404"
        );
        AddQ(
            "Как задать метаданные страницы в App Router?",
            [
        ...
        AddQ(
            "Какая библиотека популярна для Drag and Drop в React?",
            ["@dnd-kit/core", "react-dnd", "dragula", "sortablejs"],
            "@dnd-kit/core"
        );
        AddQ(
            "Какой компонент делает элемент перетаскиваемым в dnd-kit?",
            ["useDraggable", "DndContext", "Draggable", "DragOverlay"],
            "useDraggable"
        );
        AddQ(
            "Какая библиотека популярна для анимаций в React?",
            ["framer-motion", "animate.css", "react-spring", "gsap"],
            "framer-motion"
        );
        AddQ(
            "Какой хук в framer-motion для анимации появления?",
            ["useAnimate", "useAnimation", "motion.div с initial/animate", "animate()"],
            "motion.div с initial/animate"
        );
        AddQ(
            "Как сделать анимацию при размонтировании в framer-motion?",
            ["AnimatePresence + exit", "onUnmount", "exitAnimation", "leave"],
            "AnimatePresence + exit"
        );
        AddQ(
            "Как сделать сортировку элементов в dnd-kit?",
            ["@dnd-kit/sortable + useSortable", "SortableContainer", "DragSort", "SortableList"],
            "@dnd-kit/sortable + useSortable"
        );
        AddQ(
            "Какой компонент отображает перетаскиваемый элемент поверх других?",
            ["DragOverlay", "DragPreview", "DragClone", "DragShadow"],
            "DragOverlay"
        );
        AddQ(
            "Какая CSS-анимация подходит для accordion?",
            [
                "height переход с overflow: hidden",
                "opacity",
                "transform scale",
                "display none/block",
            ],
        ...
        AddQ(
            "Как анимировать переход между страницами в Next.js?",
            [
                "framer-motion + AnimatePresence в layout",
                "CSS transitions",
                "next/router events",
                "page transitions",
            ],
        ...
        AddQ(
            "Что такое layout animation в framer-motion?",
            [
                "Анимация при перестановке элементов с layout prop",
                "Анимация макета",
                "Анимация сетки",
                "CSS Grid анимация",
            ],
        ...
        var savedLectures = await db
            .Lectures.Where(l => l.CourseId == course.Id)
            .OrderBy(l => l.Order)
            .ToListAsync();
        var savedCourseTests = await db
            .Tests.Where(t => t.CourseId == course.Id)
            .OrderBy(t => t.Title)
            .ToListAsync();
