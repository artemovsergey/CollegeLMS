...
### UC-69: Система собирает базовую статистику посещений

**Acceptance Criteria:**
- [ ] Парсер nginx access logs ежедневно загружает данные в БД (IP, timestamp, URL, User-Agent, реферер, статус, время ответа)
- [ ] IP анонимизируется после определения страны/города (сохраняется только регион)
- [ ] Агрегация: количество уникальных посетителей, просмотры страниц, RPS, 4xx/5xx
- [ ] Хранение данных: минимум 6 месяцев с ежемесячной ротацией

**API:** Нет (внутренний процесс)

**Статус:** ❌ Не реализовано

### UC-70: Система собирает клиентские события через JS

**Acceptance Criteria:**
- [ ] JS-сниппет встроен во все страницы сайта (через layout/header)
- [ ] События: pageview, click, scroll depth, поисковый запрос
- [ ] POST /api/analytics/event — приём событий (анонимно: sessionId, eventType, payload, timestamp)
- [ ] Rate limiting: не более 100 событий в минуту с одного sessionId
- [ ] Сниппет не блокирует загрузку страницы (defer/async)
- [ ] Не собирает PII (персональные данные)
...
**Acceptance Criteria:**
- [ ] GET /api/analytics/dashboard — сводка: посетители (день/неделя/месяц), просмотры, RPS, 4xx/5xx
- [ ] Графики: посещения по дням, устройства (desktop/mobile/tablet), источники трафика
- [ ] Таблица популярных страниц (URL, просмотры, среднее время)
- [ ] Фильтр по дате (с — по)
- [ ] AUTH-2 (только Admin)
- [ ] UI-1, UI-2, UI-3

**API:** `GET /api/analytics/dashboard?from={date}&to={date}`

**Статус:** ❌ Не реализовано

### UC-72: Администратор просматривает статистику поиска

**Acceptance Criteria:**
- [ ] GET /api/analytics/search — топ поисковых запросов (query, count, результатов найдено)
- [ ] Запросы без результатов (query, count) — чтобы улучшить поиск
- [ ] UI-аудит существующего поиска на сайте: виден ли компонент, автоподсказки, пустые состояния, релевантность результатов, удобство на мобильных
- [ ] Если поиска нет — спроектировать и добавить компонент поиска
- [ ] AUTH-2 (только Admin)
- [ ] UI-1, UI-2, UI-3

**API:** `GET /api/analytics/search?from={date}&to={date}`

**Статус:** ❌ Не реализовано

### UC-73: Администратор экспортирует отчёт по посещаемости

**Acceptance Criteria:**
- [ ] GET /api/analytics/export?format=pdf — PDF-отчёт (графики посещений, таблица устройств, источники)
- [ ] GET /api/analytics/export?format=xlsx — Excel-отчёт (сырые данные по дням)
- [ ] Параметры: период (с — по), при необходимости фильтр по странице/разделу
- [ ] AUTH-2 (только Admin)
- [ ] UI-1, UI-3

**API:** `GET /api/analytics/export?from={date}&to={date}&format={pdf|xlsx}`
...
| P1 | AnalyticsService | UC-69..73 | ❌ Не начато |
...
| AnalyticsService | 5 (UC-69 — UC-73) | ❌ |
...
22. **Добавлен AnalyticsService (UC-69..73)** — сервис аналитики сайта (P1, section 17):
     ...
     - UC-71: администратор просматривает дашборд аналитики
     - UC-72: администратор просматривает статистику поиска + UI-аудит
     - UC-73: администратор экспортирует отчёт
     - **Итого: 73 User Stories** (было 68)
23. **Актуализация статусов (2026-07-09, v2)**:
