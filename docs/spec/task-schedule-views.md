...
- `pairCount` — число пар по правилам §5. Для группы: день УП → число пар УП, день ПП → `0`;
  для преподавателя — число обычных пар дня + число пар УП, при этом ПП исключается из
  `practiceKinds`/`practiceName` и пар не даёт. `0` в воскресенье, нерабочий день и вне семестра.
  Сетка 7×N (ведущие/замыкающие дни) достраивается на клиенте.
...
4. **Практика** (для группы — по `GroupId`, для преподавателя — по `TeacherId`, дата в периоде):
   - **перспектива группы**: `practices[]` — карточки УП/ПП; в день УП `entries` = синтезированные
     пары УП по `PracticeDay.PairNumbers` (`LessonType=Practice`, `isPractice=true`, `practiceName`,
     `Subject=Name`, время из звонков, emerald), обычные пары и `inserts` подавляются; в день ПП
     `entries` и `inserts` пусты;
   - **перспектива преподавателя**: `practices[]` пуст; в день УП `entries` = обычные пары + пары УП
     (на одно время — отдельные строки), `inserts` обычные; ПП скрыта полностью, обычные пары дня
     отдаются как обычно.
5. **Обычный день**:
...
с учётом перспективы (группа/преподаватель).
...
- **День** (`DayFeed`): блок «Вставки», карточка практики группы (в день УП дополнительно пары УП
  из `entries`; у преподавателя карточек практик нет — только пары), «Нерабочий день: {название}»,
...
| UC-SCH-17 | Практики поверх расписания: перспективы группы и преподавателя, пары УП, маркеры в календаре/семестре |
