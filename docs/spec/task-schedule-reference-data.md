...
- **Практики** — перспектива группы: в день УП `entries` содержат только пары УП, синтезированные по
  `PracticeDay.PairNumbers` (`LessonType=Practice`, `isPractice=true`, `practiceName`, `Subject=Name`,
  время из звонков, emerald-подсветка), обычные пары и вставки подавляются; слой `practices[]`
  отдаётся как раньше — карточка УП/ПП (ПП — amber). В день ПП карточка практики есть, пары отсутствуют.
  Перспектива преподавателя: слой `practices[]` пуст; в день УП `entries` = обычные пары + пары УП
  (УП-пара и обычная пара на одно время — отдельные строки), в день ПП обычные пары дня видны как
  обычно, а ПП не отображается (ни карточка, ни пары); суббота попадает в неделю преподавателя только
  при УП. Уведомления бота `PracticeNotifier` не затронуты.
