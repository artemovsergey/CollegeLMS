...
F5:F6: "№"                           → № пары
G5:G6: "Примеч."                     → примечание; паттерн «вм.X» (напр. «вм.4 п») = старая пара вводимого занятия D/E
```

### 2.2 Определение операции по заполненности

Дополнительные признаки: F = № пары строки (цель), G = примечание; паттерн «вм.X» означает, что вводимое занятие D/E переносится со старой пары X.

| Заполнено | Примечание | Операции |
|-----------|------------|----------|
| только `B/C` | любое | **Remove** — снять B/C с пары F |
| только `D/E` | без «вм.X» | **Add** — ввести D/E на пару F |
| только `D/E` | «вм.X» | **Replace** — снять D/E с пары X, ввести D/E на пару F (1 запись) |
| `B/C` и `D/E` | без «вм.X» | **Replace** — заменить содержимое пары F: снять B/C, ввести D/E (1 запись) |
| `B/C` и `D/E` | «вм.X» | 2 записи: **Replace** на паре F (снять B/C, ввести D/E) + **Remove** — снять D/E с пары X |

Замена содержимого слота (снятие и ввод на одной паре F) допустима; «пара снятия и ввода одинакова» не является ошибкой.

Поиск снимаемого занятия в Replace выполняется по группе+дню+неделе+паре+предмету+преподавателю: для заявленной замены — по паре F и B/C; для переноса «вм.X» — по паре X и D/E.

Одна строка может породить несколько записей превью (случай «B/C и D/E» + «вм.X»); каждая запись несёт собственный `NumberPair`/`RemovedNumberPair`.
...
| Replace: снимаемое занятие не найдено по паре+предмету+преподавателю | «Строка N: занятие на {день} {неделя}-й неделе, пара {M} не найдено.» |
