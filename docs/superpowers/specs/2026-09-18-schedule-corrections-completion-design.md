...
| Применение | Две фазы: (1) симуляция без мутаций → `400` со списком «Строка N: сообщение»; (2) транзакция с трекингом, защита от гонки conditional UPDATE `Draft→Applied` (0 строк → `409`) |
...
- `appliedAt: DateTime?`, `appliedByUserId: Guid?`, `appliedByName: string?`;
- `positions[].errors` — ошибки конкретной позиции (подмножество).
