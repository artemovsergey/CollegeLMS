...
- **Replace:** поиск снимаемого занятия по группе+дню+неделе+паре+предмету B+преподавателю C. Замена содержимого слота (пара снятия == пара ввода F) ДОПУСТИМА — это не ошибка. Примечание «вм.X» переносит вводимое D/E со старой пары X: строка превращается в 2 записи (Replace на F + Remove на X); только D/E с «вм.X» — в 1 запись Replace (снять D/E с X, ввести на F).
...
- **Пара снятия и ввода одинакова**: по решению пользователя — ВАЛИДНО (замена содержимого слота). Обновлено: проверка удалена (см. строку 24); строка 9 эталонного `Корректировка.xlsx` (Физика→Математика на паре 3) теперь разбирается как замена слота.
